...
class Encoder_M(nn.Module):
    def __init__(self):
        ...
        self.conv1_om = nn.Conv2d(
            1, 64, kernel_size=7, stride=2, padding=3, bias=False
        )  # other mask (not include bg)
        self.conv1_n = nn.Conv2d(
            1, 64, kernel_size=7, stride=2, padding=3, bias=False
        )  # other scribble (include bg)
    ...
    def forward(self, in_f, in_m, in_s, in_os):
        ...
        x = (
            self.conv1(f) + self.conv1_m(m) + self.conv1_p(s) + self.conv1_n(os)
        )  # + self.conv1_om(om)
        x = self.bn1(x)
...
class IGN(nn.Module):
    ...
    def memorize(self, frame, masks, scribs):
        ...
        # pad
        (frame, masks, scribs), pad = pad_divide_by(
            [frame, masks, scribs], 16, (frame.size()[2], frame.size()[3])
        )
    ...
    def segment(self, frames, keys, values):
        ...
        # pad
        [frames], pad = pad_divide_by(
            [frames], 16, (frames.size()[2], frames.size()[3])
        )
